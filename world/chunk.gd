...
func regenerate():
	# Wait for any existing thread to finish before regenerating
	if _thread and _thread.is_alive():
		_thread.wait_to_finish()
	
	# Clear out all old nodes first - use immediate removal for meshes
	...
	_generate_chunk_collider()
	# Use threading for mesh regeneration to avoid stuttering
	_thread = Thread.new()
	_thread.start(_generate_chunk_mesh)
...
func _generate_chunk_collider():
	...
	for block_position in data.keys():
		...
		# Don't create colliders for transparent blocks (bushes and ladders)
		if block_id != 27 and block_id != 28 and block_id != 4:
...
func _generate_chunk_mesh():
	...
	# Check if we're in a thread - if so, use call_deferred, otherwise add directly
	if _thread and _thread.is_alive():
...
static func is_block_transparent(block_id):
	return block_id == 0 or (block_id > 25 and block_id < 30) or block_id == 4  # Ladder blocks are transparent
